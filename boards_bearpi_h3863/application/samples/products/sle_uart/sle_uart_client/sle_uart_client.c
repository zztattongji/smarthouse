...
/* =========================
   ...
   '2' -> 闪烁
   ========================= */
void sle_uart_client_parse_led_cmd(const uint8_t *buf, uint16_t len)
{
    if (buf == NULL || len == 0) {
        return;
    }
    ...
    switch (buf[0]) {
        case '0':
        case 0x00:
            osal_printk("[LED CMD] OFF\r\n");
            led_ctrl_set_mode(LED_MODE_OFF);
            break;

        case '1':
        case 0x01:
            osal_printk("[LED CMD] ON\r\n");
            led_ctrl_set_mode(LED_MODE_ON);
            break;

        case '2':
        case 0x02:
            osal_printk("[LED CMD] BLINK\r\n");
            led_ctrl_set_mode(LED_MODE_BLINK);
            break;

        default:
            osal_printk("[LED CMD] Unknown cmd: 0x%02x\r\n", buf[0]);
            break;
    }
}
