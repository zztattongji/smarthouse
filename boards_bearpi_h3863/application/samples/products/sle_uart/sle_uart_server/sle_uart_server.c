...
/* sle connect acb handle — 多连接支持: 最多 MAX_SLE_CLIENTS 个 Client */
...
static int conn_table_add(uint16_t conn_id)
{
    ...
    if (conn_table_find(conn_id) >= 0) return 0;  /* already exists */
...
errcode_t sle_uart_server_send_report_by_uuid(const uint8_t *data, uint8_t len)
{
    ...
    osal_vfree(param.value);
    return (g_sle_conn_count > 0) ? last_ret : ERRCODE_SLE_FAIL;
}

/* device通过handle向host发送数据：report — 广播到所有已连接 Client */
