...
/* sle connect acb handle -- support multiple clients */
#define MAX_SLE_CLIENTS 8
static uint16_t g_sle_conn_hdls[MAX_SLE_CLIENTS];
...
{
    return (g_sle_conn_count > 0) ? g_sle_conn_hdls[0] : 0;
}

static int conn_table_find(uint16_t conn_id)
{
    for (uint8_t i = 0; i < g_sle_conn_count; i++) {
        if (g_sle_conn_hdls[i] == conn_id) return (int)i;
    }
    return -1;
}

static int conn_table_add(uint16_t conn_id)
{
    if (g_sle_conn_count >= MAX_SLE_CLIENTS) return -1;
    if (conn_table_find(conn_id) >= 0) return 0;
    g_sle_conn_hdls[g_sle_conn_count++] = conn_id;
    return 0;
}

static void conn_table_remove(uint16_t conn_id)
{
    int idx = conn_table_find(conn_id);
    if (idx < 0) return;
    for (uint8_t i = (uint8_t)idx; i < g_sle_conn_count - 1; i++) {
        g_sle_conn_hdls[i] = g_sle_conn_hdls[i + 1];
    }
    g_sle_conn_count--;
}
...
{
    if (data == NULL || len == 0 || g_sle_conn_count == 0) {
        return ERRCODE_SLE_FAIL;
    }
    errcode_t ret;
    ...
    errcode_t last_ret = ERRCODE_SLE_FAIL;
    for (uint8_t i = 0; i < g_sle_conn_count; i++) {
        ret = ssaps_notify_indicate_by_uuid(g_server_id, g_sle_conn_hdls[i], &param);
        if (ret != ERRCODE_SLE_SUCCESS) last_ret = ret;
        else last_ret = ERRCODE_SLE_SUCCESS;
    }
    if (last_ret != ERRCODE_SLE_SUCCESS) {
        osal_vfree(param.value);
        return last_ret;
    }
    osal_vfree(param.value);
...
{
    if (data == NULL || len == 0 || len > UART_BUFF_LENGTH || g_sle_conn_count == 0) {
        return ERRCODE_SLE_FAIL;
    }
    ...
    errcode_t last_ret = ERRCODE_SLE_FAIL;
    for (uint8_t i = 0; i < g_sle_conn_count; i++) {
        errcode_t ret = ssaps_notify_indicate(g_server_id, g_sle_conn_hdls[i], &param);
        if (ret != ERRCODE_SLE_SUCCESS) last_ret = ret;
        else last_ret = ERRCODE_SLE_SUCCESS;
    }
    return last_ret;
    ...
    sle_acb_state_t conn_state, sle_pair_state_t pair_state, sle_disc_reason_t disc_reason)
{
    ...
    if (conn_state == SLE_ACB_STATE_CONNECTED) {
        (void)conn_table_add(conn_id);
    } else if (conn_state == SLE_ACB_STATE_DISCONNECTED) {
        conn_table_remove(conn_id);
        if (g_sle_conn_count == 0) g_sle_pair_hdl = 0;
